...
const ProgrammerIntervention = () => {
  ...
  const calculateNextMaintenance = (currentKilometrage, selectedPieces) => {
    ...
    const intervals = selectedPieces.map((piece) => maintenanceData.Maintenances[piece.value].intervalKm);
  ...
  const handlePiecesChange = (selectedOptions) => {
    ...
    if (selectedEquipement) {
      const nextMaintenance = calculateNextMaintenance(parseInt(kilometrage), selectedOptions);
  ...
  const maintenanceOptions = Object.keys(maintenanceData.Maintenances).map((key) => ({
    value: key,
